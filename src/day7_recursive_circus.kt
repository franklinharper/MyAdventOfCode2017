...
fun main(args: Array<String>) {
    ...
    input.reader().forEachLine {
        val (name, weightString) = it.split(whitespace)
        val weight = Integer.parseInt(weightString.substring(1, weightString.length - 1))
        val program = Program(name, weight, ArrayList<Program>(), 0)
        programMap[name] = program
    }
    input.reader().forEachLine {
        val words = it.split(whitespace)
        val parent = programMap[words[0]]!!
        for (i in 3 until words.size) {
            val belowName = words[i].split(',')[0]
//            println("${words[0]} is children $belowName")
            val below = programMap[belowName]
//            println("children: $children")
            parent.children.add(below!!)
        }
    }

    val m = HashMap(programMap)
    for (p in programMap.values) {
        for (below in p.children) {
            m.remove(below.name)
        }
    }
    val root = m.values.first()
    println("${m.size}, $m")
    rebalance(root)
    println("programs:$root")
}

fun rebalance(node: Program) {
    if (node.children.size == 0) {
        // leaf
        node.weightSum = node.weight
    } else {
        val children = node.children
        children.forEach { rebalance(it) }
        if (children.size == 1) {

        } else if (children.size == 2) {

        } else {
            if (children[0].weightSum == children[1].weightSum) {
                val correctWeightSum = children[0].weightSum
                for (i in 2 until children.size) {
                    if (children[i].weightSum != correctWeightSum) {
                        printCorrection(correctWeightSum, children[i])
                    }
                }
            } else {
                if (children[0].weightSum == children[2].weightSum) {
                    val correctWeightSum = children[0].weightSum
                    printCorrection(correctWeightSum, children[1])
                } else {
                    val correctWeightSum = children[1].weightSum
                    printCorrection(correctWeightSum, children[0])
                }
            }
        }
        node.weightSum = node.weight
        children.forEach { node.weightSum += it.weightSum }
    }
}

private fun printCorrection(correctWeightSum: Int, p: Program) {
    val correction = correctWeightSum - p.weightSum
    val correctedWeight = p.weight + correction
    println("child ${p.name}'s weight is ${p.weight}")
    println("child ${p.name}'s corrected weight is $correctedWeight")
}
