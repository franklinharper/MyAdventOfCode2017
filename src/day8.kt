...
""".trimIndent()
    ...
    var variableValues = HashMap<String, Int>()

//    for (word in phrase.split(whitespace)) {

    fullInput.reader().forEachLine {
        val instruction = it.split(whitespace)
        val testVariable = getValue(variableValues, instruction[4])
        val testOperator = instruction[5]
        val testOperand = Integer.parseInt(instruction[6])
        if (testOperator == ">") {
            if (testVariable > testOperand) {
                execute(variableValues, instruction[0], instruction[1], instruction[2])
            }
        }
    ...
    variableValues.values.forEach {
        if (it > max) {
            max = it
        }
    }
    println(max)
    println(maxIntermediate)
}

var maxIntermediate = Integer.MIN_VALUE

fun execute(variableValues: HashMap<String, Int>, variableName: String, operator: String, operand: String): Unit {
    if (operator == "inc") {
        variableValues[variableName] = getValue(variableValues, variableName) + Integer.parseInt(operand)
    } else if (operator == "dec") {
        variableValues[variableName] = getValue(variableValues, variableName) - Integer.parseInt(operand)
    }
    if (variableValues[variableName]!! > maxIntermediate ) {
        maxIntermediate = variableValues[variableName]!!
    }
}

fun getValue(variableValues: HashMap<String, Int>, variableName:String): Int {
    if (variableValues[variableName] == null) {
        return 0;
    } else {
